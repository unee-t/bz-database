...
# 
# This script will:
#	- Create a temporary table with all the all units in the table 'ut_data_to_create_units' which have not been created yet (product_id is NULL)
# 	- Create a procedure which will loop and, for all these units
...
# Info about this script
	SET @script = '2_Mass_Insert_new_unit_with_dummy_roles_in_unee-t_bzfe_v3.0.sql';
...
# We create the table which list the units we need to process:
	/*Table structure for table `ut_temp_data_to_create_units` */
		DROP TABLE IF EXISTS `ut_temp_data_to_create_units`;

		CREATE TABLE `ut_temp_data_to_create_units` (
		  `unit_to_create_token` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'token of the Unit to create',
		  `unit_to_create_id` INT(11) NOT NULL COMMENT 'The ID in the table `ut_data_to_create_units`',
		  PRIMARY KEY (`unit_to_create_token`)
		) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
		
	# We populate this table with the data where there are now information
		INSERT INTO `ut_temp_data_to_create_units`
			(`unit_to_create_id`
			)

		SELECT `id_unit_to_create`
		FROM `ut_data_to_create_units`
		WHERE `product_id` IS NULL
		ORDER BY `id_unit_to_create` ASC
		;

# How many records do we need to process?
	SET @max_loops_mass_import_units = (SELECT MAX(`unit_to_create_token`) FROM `ut_temp_data_to_create_units`);
...
DELIMITER &&
DROP PROCEDURE IF EXISTS mass_insert_new_unit_dummy_roles&&
CREATE PROCEDURE mass_insert_new_unit_dummy_roles()
BEGIN
DECLARE number_of_loops INT DEFAULT 1;
WHILE number_of_loops < (@max_loops_mass_import_units +1) DO


# The record that we need to process in this loop
	SET @record_to_process_mass_import_new_unit = (SELECT `unit_to_create_id` FROM `ut_temp_data_to_create_units` WHERE `unit_to_create_token` = number_of_loops);

# Timestamp	
	SET @timestamp = NOW();

# The procedure to create the units which exist in the table 'ut_temp_data_to_create_units'

# The unit:

	# BZ Classification id for the unit that you want to create (default is 2)
	SET @classification_id = (SELECT `classification_id` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);

	# The name and description
	SET @unit_name = (SELECT `unit_name` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_identification = (SELECT `unit_id` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_condo = (SELECT `unit_condo` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_surface = (SELECT `unit_surface` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_surface_measure = (SELECT `unit_surface_measure` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_description_details = (SELECT `unit_description_details` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @unit_address = (SELECT `unit_address` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
	SET @matterport_url = (SELECT `matterport_url` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
...
# The users associated to this unit.	
	...
	SET @creator_bz_id = (SELECT `bzfe_creator_user_id` FROM `ut_data_to_create_units` WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit);
...
# Update the table 'ut_data_to_create_units' so that we record that the unit has been created
	...
	WHERE `id_unit_to_create` = @record_to_process_mass_import_new_unit;
...
# Call the procedure to mass insert the units in the BZ db.
	CALL mass_insert_new_unit_dummy_roles;	
		
# We drop the table which list the units we needed to create.
	DROP TABLE IF EXISTS `ut_temp_data_to_create_units`;
