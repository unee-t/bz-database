...
#
# The logic for this script is to look in the table 'ut_product_group':
# All the products where:
#	- component_id IS NOT NULL
# AND
#	- group_type_id = 5
#
# are the product_id that we need to work with...
#
#################################################################
#																#
...
# Info about this script
	SET @script = 'add_missing_roles_with_generic_users_multiple_units_bzfe_v2.18.sql';
...
# We need to list the products where there is no group to restrict access to the units
	...
	/*Table structure for table `ut_lmb_units_that_are_missing_roles` */
	DROP TABLE IF EXISTS `ut_lmb_units_that_are_missing_roles`;

	CREATE TABLE `ut_lmb_units_that_are_missing_roles` (
	...
	INSERT INTO `ut_lmb_units_that_are_missing_roles`
		(`product_id`)
		SELECT `product_id` FROM `ut_product_group` 
			WHERE `component_id` IS NOT NULL
			AND `group_type_id` = 5
			;

# How many records do we need to process?
	SET @max_loops = (SELECT MAX(`id_record`) FROM `ut_lmb_units_that_are_missing_roles`);

DELIMITER $$
	DROP PROCEDURE IF EXISTS insert_missing_roles_lmb_unit$$
	CREATE PROCEDURE insert_missing_roles_lmb_unit()
		BEGIN
		DECLARE number_of_loops INT DEFAULT 1;
		WHILE number_of_loops < (@max_loops +1) DO

	# BZ product_id for the unit
		SET @product_id = (SELECT `product_id` FROM `ut_lmb_units_that_are_missing_roles` WHERE `id_record` = number_of_loops);
...
# Call the procedure to update the LMB units.
	CALL insert_missing_roles_lmb_unit;	
