...
#
# WARNING! THIS SCRIPT IS USED IN THE SCRIPT 'invitation_as_assignee_at_case_creation_v3.0'
#	MAKE SURE THAT YOU ALSO UPDATE 'invitation_as_assignee_at_case_creation_v3.0' IF YOU UPDATE THIS SCRIPT
#
#
# Pre-requisite:
...
# Info about this script
	SET @script = '3_mass_replace_dummy_role_with_genuine_user_as_default_in_unee-t_bzfe_v3.0.sql';
...
# We create the table which list the records we need to process:
	/*Table structure for table `ut_temp_data_to_replace_dummy_roles` */
		DROP TABLE IF EXISTS `ut_temp_data_to_replace_dummy_roles`;

		CREATE TABLE `ut_temp_data_to_replace_dummy_roles` (
		  `record_to_process_token` INT(11) NOT NULL AUTO_INCREMENT COMMENT 'token of the record to process',
		  `record_to_process_id` INT(11) NOT NULL COMMENT 'The ID in the table `ut_data_to_create_units`',
		  PRIMARY KEY (`record_to_process_token`)
		) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;
		
	# We populate this table with the data where there are now information
		INSERT INTO `ut_temp_data_to_replace_dummy_roles`
			(`record_to_process_id`
			)

		SELECT `id`
		FROM `ut_data_to_replace_dummy_roles`
		WHERE `bz_created_date` IS NULL
		ORDER BY `id` ASC
		;

# How many records do we need to process?
	SET @max_loops_mass_replase_dummy_user = (SELECT MAX(`record_to_process_token`) FROM `ut_temp_data_to_replace_dummy_roles`);
...
# We prepare all the procedures we will need in the loop:
...
# This is the start of the loop to replace all the dummy users for the units we have not processed yet

DELIMITER &&
DROP PROCEDURE IF EXISTS mass_replace_dummy_roles_with_genuine_users&&
CREATE PROCEDURE mass_replace_dummy_roles_with_genuine_users()
BEGIN
DECLARE number_of_loops INT DEFAULT 1;
WHILE number_of_loops < (@max_loops_mass_replase_dummy_user +1) DO
...
# The record that we need to process in this loop
	SET @record_to_process_mass_replace_dummy_unit = (SELECT `record_to_process_id` FROM `ut_temp_data_to_replace_dummy_roles` WHERE `record_to_process_token` = number_of_loops);
		
# The unit:
	
	# The name and description
		SET @product_id = (SELECT `bz_unit_id` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);
...
# The user associated to the first role in this unit.	
	...
	# For LMB migration, we use 2 (support.nobody)
		SET @creator_bz_id = (SELECT `bzfe_invitor_user_id` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);

	# BZ user id of the user that you want to associate to the unit.
		SET @bz_user_id = (SELECT `bz_user_id` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);
	...
	#	- Contractor 3
	#	- Management company 4
		SET @id_role_type = (SELECT `user_role_type_id` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);
		SET @role_user_more = (SELECT `user_more` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);

	# Is the BZ user an occupant of the unit?
		SET @is_occupant = (SELECT `is_occupant` FROM `ut_data_to_replace_dummy_roles` WHERE `id` = @record_to_process_mass_replace_dummy_unit);
...
# Update the table 'ut_data_to_replace_dummy_roles' so that we record what we have done
	...
	WHERE `id` = @record_to_process_mass_replace_dummy_unit;
...
# Call the procedure to mass insert the units in the BZ db.
	CALL mass_replace_dummy_roles_with_genuine_users;		
